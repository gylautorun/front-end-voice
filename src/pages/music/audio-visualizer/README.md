...
flowchart LR
    ...
    LyricsQuery --> LyricsCache[本地同步歌词缓存]
    LyricsCache -->|命中| LyricsPanel[左侧同步歌词面板]
...
2. 歌手完全一致或互相包含。
3. 网络歌曲时长与本地解码时长接近。
4. 只保留包含 `syncedLyrics` 且能解析出有效时间标签的结果。
5. 纯音乐或时长差距过大的候选会被扣分。
...
`LyricLine` 类型仍允许 `null`，供纯文本解析器和未来识别链路复用；当前网络查询、缓存和面板只接受 `startTime` 为有效数字的同步歌词。
...
同步歌词行使用按钮语义，点击后调用播放器的 `seek()` 跳转。接口只有全文歌词或时间标签解析失败时进入 `not-found`，不会再渲染无法准确同步的歌词版本。
...
- `idle`：等待音频解码。
- `loading`：正在匹配。
- `success`：展示带有效时间轴的同步歌词。
- `not-found`：服务可用，但没有可解析的同步歌词候选。
- `error`：网络、服务状态或请求频率错误。

### 9.8 本地缓存

`lyrics-cache.ts` 使用 `localStorage` 保存已经解析和校验的同步歌词：

- 缓存键由标准化搜索词和四舍五入后的音频时长组成。
- 查询先读缓存，命中后不再访问 LRCLIB。
- 网络请求成功后立即写入缓存，面板显示“已缓存”状态。
- 缓存有效期为 30 天，最多保留最近使用的 30 首歌曲。
- 读取时重新验证字段、时间轴和正文，损坏或旧版本数据会被忽略。
...
解决：候选阶段直接过滤全文歌词，只接受能解析出时间轴的版本；成功结果按歌曲和时长持久缓存，之后优先从本地恢复。同步歌词继续按当前行居中滚动，并保留首尾居中空间。
...
- 网络歌词使用歌曲名、歌手和时长匹配，只接受真实 LRC 时间轴，并通过有界、带有效期的本地缓存减少重复请求。
