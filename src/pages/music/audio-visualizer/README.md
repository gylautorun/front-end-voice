...
- 展示七段彩色频谱图例。
- 根据文件名、歌手和音频时长查询网络歌词，优先时间轴版本并回退到全文。
- 支持手动关键词搜索、同步高亮、点击跳转和本地持久缓存。
- 宽屏将歌词固定在工作区左侧，窄屏自动移动到播放器下方。
- 切换三种 Canvas 可视化模式：
  - 频谱柱。
  - 中线对称的镜像窄柱波形。
  - 球形水滴波纹。
...
├── lyrics/
│   ├── types.ts                      # 歌词行、结果和查询状态类型
│   ├── lrc-parser.ts                 # LRC 时间轴和全文歌词解析
│   ├── lyrics-cache.ts               # 同步歌词 localStorage 缓存与校验
│   ├── lrclib-client.ts              # 网络请求、候选评分和结果标准化
│   └── use-lyrics.ts                 # 缓存优先查询、取消旧请求和状态管理
├── components/
│   ├── visualizer-toolbar.tsx        # 音轨信息、模式切换、文件选择
│   ├── visualizer-stage.tsx          # Canvas 舞台、状态和解析遮罩
│   ├── player-controls.tsx           # 播放、进度和音量控件
│   ├── lyrics-panel.tsx              # 搜索、同步高亮和歌词滚动
...
flowchart LR
    ...
    Metadata --> LyricsQuery[歌曲名 / 歌手 / 时长]
    LyricsQuery --> LyricsCache[本地歌词缓存]
    LyricsCache -->|命中| LyricsPanel[左侧歌词面板]
    LyricsCache -->|未命中| LRCLIB[LRCLIB 网络歌词]
    LRCLIB --> LyricsCache
...
| `LyricsPanel` | 左侧歌词搜索、同步高亮、自动滚动和点击跳转 |
...
- `lyrics/lrclib-client.ts`
- `lyrics/lrc-parser.ts`
- `lyrics/lyrics-cache.ts`
- `lyrics/use-lyrics.ts`
- `components/lyrics-panel.tsx`

### 9.1 为什么先选择网络歌词

实时语音识别需要将整段音乐上传到后端模型，并且通常还要先做人声分离。对于已发行歌曲，网络歌词具有这些优势：

- 不上传完整音频，只发送歌曲关键词和时长。
- 响应更快且没有逐分钟识别成本。
- 已有 LRC 时可以直接获得人工校对的时间轴。
- 背景音乐、和声、混响不会降低歌词文本准确率。

当前使用 LRCLIB 公共接口：

```text
GET https://lrclib.net/api/search?q=<关键词>
```

该接口不需要前端密钥，并允许浏览器跨域请求。当前页面只向服务发送搜索词，不上传用户的音频文件。

### 9.2 自动搜索时机

音频刚选中时只有文件名，还没有精确时长。歌词 Hook 会等待 `decodeAudioData()` 完成，再执行自动搜索：
...
这样避免同一文件请求两次，也能借助时长区分原版、Live、伴奏和剪辑版本。
...
```text
张申骋 - 何惜一战.mp3 -> 张申骋 - 何惜一战
春涧.mp3              -> 春涧
```

对于“歌手 - 歌名”格式，候选评分会分别比较歌手和歌曲名。只有歌曲名时仍可搜索，但同名歌曲的误配概率更高，因此界面保留手动搜索框用于补充歌手或版本信息。

### 9.4 候选评分

搜索可能返回多个同名结果，当前评分优先级为：

1. 歌曲名完全一致。
2. 歌手完全一致或互相包含。
3. 网络歌曲时长与本地解码时长接近。
4. 优先选择能解析出有效时间标签的 `syncedLyrics`。
5. 纯音乐或时长差距过大的候选会被扣分。

如果所有匹配候选都没有可解析时间轴，则从相同的评分顺序中选择第一份有效 `plainLyrics`。这样不会因为服务只收录全文歌词而把已有内容误判为未找到。

歌曲名和歌手比较前会统一大小写，并去除空白、标点和符号，降低不同命名格式造成的影响。

歌名标准化后还必须相等或互相包含。时长只能帮助选择同一歌曲的不同版本，不能单独使一个无关候选通过匹配。
...
- `[01:23.450]` 三位毫秒时间。
- 同一文本行具有多个时间标签。
- `[offset:+500]` 或 `[offset:-500]` 整体时间修正。
- 源码顺序与播放时间不一致时重新排序。
...
同步歌词的 `startTime` 是有效秒数；全文歌词没有可靠时间轴，因此保存为 `null`。缓存读取时会根据 `isSynced` 分别校验这两种结构。

### 9.6 同步高亮和滚动

同步歌词按起始时间升序排列。每次播放进度更新时使用二分查找定位当前行：

```text
目标 = startTime <= currentTime 的最后一行
```

相比每次从头遍历，二分查找复杂度为 `O(log n)`。

当前行变化后，歌词面板根据元素 `offsetTop` 计算容器内部的居中位置，并调用 `scrollTo()` 平滑滚动。同步歌词正文前后各保留半个可视区的弹性空间，因此第一行和最后一行也能移动到中线。滚动只作用于歌词区域，不会移动整个页面。

同步歌词行使用按钮语义，点击后调用播放器的 `seek()` 跳转。全文歌词显示在同一个左侧歌词正文区，但不提供不准确的逐行高亮和点击跳转；组件使用 `currentTime / duration` 将整首播放进度映射到正文可滚动距离。

### 9.7 请求并发和错误状态

切换音频或重复搜索时，`useLyrics` 会通过 `AbortController` 取消上一次请求，避免旧歌曲歌词覆盖新歌曲。

单次请求超过 12 秒会自动终止并进入错误状态，避免接口无响应时永久显示加载动画。

界面区分：

- `idle`：等待音频解码。
- `loading`：正在匹配。
- `success`：展示同步歌词或作为兜底的全文歌词。
- `not-found`：服务可用，但同步和全文候选都没有有效正文。
- `error`：网络、服务状态或请求频率错误。

### 9.8 本地缓存

`lyrics-cache.ts` 使用 `localStorage` 保存已经解析和校验的同步或全文歌词：

- 缓存键由标准化搜索词和四舍五入后的音频时长组成。
- 查询先读缓存，命中后不再访问 LRCLIB。
- 网络请求成功后立即写入缓存，面板显示“已缓存”状态。
- 缓存有效期为 30 天，最多保留最近使用的 30 首歌曲。
- 读取时重新验证字段、同步时间轴或全文空时间值以及正文，损坏数据会被忽略。
- 隐私模式、权限限制或存储满额只会禁用缓存，不影响联网查询和播放。

### 9.9 语音识别兜底的边界

纯前端的 `SpeechRecognition` 主要面向麦克风，不能稳定识别 `<audio>` 元素播放的内容，因此没有作为兜底方案。
...
该方案需要明确模型服务、API 密钥、文件大小限制、处理进度、成本和音频隐私策略后再实现。
...
### 10.9 Canvas 在高分屏模糊或缩放后尺寸不正确

问题：只设置 CSS 宽高时，Canvas 物理缓冲区可能不足；侧栏和窗口变化也会改变可用尺寸。

解决：使用 `ResizeObserver` 和受限 DPR 同步物理尺寸，再用 `setTransform()` 恢复 CSS 像素坐标。

### 10.10 全文歌词无法准确同步且重复消耗网络请求

问题：全文歌词没有逐行时间标签，只能估算滚动位置；同一歌曲重复打开时还会再次请求公共接口。

解决：先在匹配候选中寻找能解析时间轴的版本，没有时再展示评分最高的全文歌词。两种成功结果都按歌曲和时长持久缓存。同步歌词按当前行居中滚动；全文歌词只按整首进度滚动，不伪造逐行时间点。

### 10.11 左侧全文歌词撑开容器而不滚动

问题：歌词栏成为 CSS Grid 子项后，全文内容的固有高度会参与自动行高计算。即使正文设置了 `overflow-y: auto`，父级仍可能被所有段落撑高，最终没有可滚动距离。

解决：歌词面板继续使用纵向 Flex 布局，将 `.lyrics-viewport` 的基础高度设为 `0`，再通过 `flex: 1 1 auto` 只分配标题和搜索框之外的剩余高度。全部歌词节点仍保留在 DOM 中，但只在受限正文视口内部滚动。
...
- 文件名和时长就绪后会自动查询一次歌词。
- 同步歌词可以高亮、自动滚动和点击跳转。
- 只有全文歌词时仍在左栏展示，并且不出现伪造的逐行高亮。
- 同一歌曲再次加载时优先命中缓存，不重复请求网络。
- 宽屏歌词位于画布左侧，窄屏位于播放器下方。
- 手动修改搜索词后可以重新匹配其他版本。
- 切换歌曲时旧歌词请求会被取消。
- 桌面、平板和手机宽度下控件不重叠。
...
歌词模块定向 ESLint、全仓 `tsc --noEmit`、Vite 生产构建和 `git diff --check` 均可通过。构建仍会输出项目现有的 Sass legacy API 和大 chunk 提示，它们不属于歌词模块的类型或运行错误。
...
- 网络歌词使用歌曲名、歌手和时长匹配，优先真实 LRC 时间轴、回退全文，并通过有界、带有效期的本地缓存减少重复请求。
